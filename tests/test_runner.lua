...
print("")
print("[Module] core.scanner")
local Scanner = require("core.scanner")
test("scan_dpkg returns table", function()
    local result = Scanner.scan_dpkg()
    assert_type(result, "table")
end)
test("scan_docker returns table", function()
    local result = Scanner.scan_docker()
    assert_type(result, "table")
end)
test("scan_binaries returns table", function()
    local result = Scanner.scan_binaries()
    assert_type(result, "table")
end)
test("scan_all returns structured results", function()
    local result = Scanner.scan_all()
    assert_type(result, "table")
    assert_true(result.dpkg_packages ~= nil, "should have dpkg_packages")
    assert_true(result.docker_containers ~= nil, "should have docker_containers")
    assert_true(result.summary ~= nil, "should have summary")
end)

print("")
print("[Module] core.takeover")
local Takeover = require("core.takeover")
test("list_symlinks returns table", function()
    local result = Takeover.list_symlinks()
    assert_type(result, "table")
end)
